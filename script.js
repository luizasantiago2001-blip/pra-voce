...
function positionBtnNo() {
  ...
  btn.style.left     = `${window.innerWidth / 2 + 60}px`;
...
function runAway(btn) {
  ...
  // Posição aleatória garantindo que o botão fique DENTRO da tela
  const newX = padding + Math.random() * (w - bw - padding * 2);
  const newY = padding + Math.random() * (h - bh - padding * 2);
  ...
  // Após 5 fugas, some de vez com graciosidade
...
function setupMusicButton() {
  // Tenta tocar música automaticamente na primeira interação do usuário
